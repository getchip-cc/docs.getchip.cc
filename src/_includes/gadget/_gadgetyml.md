...
Gadget.yml is where you define resources available to the container, issue commands to the running container, specify which containers to run in what order, and more. For example, the [*hello world*](/gadget.html#hello-world) demo uses a gadget.yml file that pulls a container available on [Docker Hub](https://hub.docker.com/).
